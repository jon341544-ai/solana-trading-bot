...
def make_api_request(method, endpoint, data=None):
    """Make authenticated API request to MIX endpoints"""
    if not config.is_configured:
        return {'error': 'API credentials not configured'}
    
    try:
        if not trading_state.is_running and endpoint not in ['/api/mix/v1/account/accounts']:
...
def get_klines(symbol='SOLUSDT_UMCBL', interval=None, limit=100):
    ...
    try:
        ...
        interval_map = {'1m': '1min', '5m': '5min', '15m': '15min', '30m': '30min', '1H': '1H', '4H': '4H', '1D': '1D'}
        granularity = interval_map.get(interval, '15min')
...
def get_current_balances():
    """Get current USDT balance from mix account"""
    try:
        # Get mix account balance
        result = make_api_request('GET', '/api/mix/v1/account/accounts?productType=UMCBL')
        ...
        if result and 'data' in result:
            ...
            if isinstance(accounts, list) and len(accounts) > 0:
                account = accounts[0]
                usdt_balance = float(account.get('usdtEquity', 0))
                
                # Get SOL positions to calculate equivalent SOL
                positions_result = make_api_request('GET', '/api/mix/v1/position/all-position?productType=UMCBL')
                if 'error' not in positions_result and 'data' in positions_result:
                    positions = positions_result['data']
                    for position in positions:
                        if position.get('symbol') == 'SOLUSDT' and position.get('holdSide') == 'long':
...
def execute_buy_order():
    ...
    try:
        ...
        # Calculate USDT value of the SOL we want to buy
        current_price = trading_state.last_signals.get('price', 0)
        if current_price == 0:
            # Get current price if not available from signals
            ticker_result = make_api_request('GET', '/api/mix/v1/market/ticker?symbol=SOLUSDT_UMCBL')
        ...
        usdt_value = config.sol_trade_amount * current_price
        
        if usdt_balance < usdt_value:
            print(f"Insufficient USDT: Need ${usdt_value:.2f}, have ${usdt_balance:.2f}")
            return False
        ...
        if 'error' not in positions_result and 'data' in positions_result:
            positions = positions_result['data']
            for position in positions:
                if position.get('symbol') == 'SOLUSDT' and position.get('holdSide') == 'short':
                    # Close short position
                    close_order = {
                        "symbol": "SOLUSDT_UMCBL",
                        "holdSide": "short",
                        "size": position.get('available'),
                        "productType": "UMCBL"
                    }
                    close_result = make_api_request('POST', '/api/mix/v1/order/close-positions', close_order)
                    if 'error' in close_result:
                        print(f"Failed to close short position: {close_result.get('message')}")
                    else:
        ...
        order_data = {
            "symbol": "SOLUSDT_UMCBL",
            "productType": "UMCBL",
            "marginMode": "crossed",  # Use cross margin
            "marginCoin": "USDT",
            "size": str(config.sol_trade_amount),
            "side": "open_long",
            "orderType": "market"
        }
        ...
        print(f"SOL Amount: {config.sol_trade_amount}")
        print(f"USDT Value: ${usdt_value:.2f}")
        print(f"Current Price: ${current_price:.2f}")
        print(f"{'='*60}\n")
        
        result = make_api_request('POST', '/api/mix/v1/order/place-order', order_data)
        
        if not trading_state.is_running:
            return False
            
        if 'error' in result:
            print(f"Market buy order failed: {result.get('message')}")
            return False
        
        print(f"✅ BUY ORDER SUCCESSFUL - LONG POSITION OPENED")
        time.sleep(2)
...
def execute_sell_order():
    ...
    try:
        ...
        if sol_balance < config.sol_trade_amount:
            print(f"Insufficient SOL position: Need {config.sol_trade_amount} SOL, have {sol_balance} SOL")
            return False
        ...
        print(f"SOL Amount: {config.sol_trade_amount}")
        print(f"Current Price: ${current_price:.2f}")
        print(f"Expected USDT: ${config.sol_trade_amount * current_price:.2f}")
        print(f"{'='*60}\n")
        ...
        if 'error' in result:
            print(f"Market sell order failed: {result.get('message')}")
            return False
        
        print(f"✅ SELL ORDER SUCCESSFUL - LONG POSITION CLOSED")
        time.sleep(2)
        get_current_balances()
...
def trading_loop():
    ...
    while trading_state.is_running:
        try:
            ...
            print(f"SOL Position: {sol_balance:.4f} | USDT Equity: ${usdt_balance:.2f}")
...
def extract_balances(balance_data):
    ...
    try:
        if balance_data and 'data' in balance_data:
            ...
            if isinstance(accounts, list) and len(accounts) > 0:
                account = accounts[0]
                usdt_balance = str(account.get('usdtEquity', '0'))
                ...
                if 'error' not in positions_result and 'data' in positions_result:
                    ...
                    for position in positions:
                        if position.get('symbol') == 'SOLUSDT' and position.get('holdSide') == 'long':
                            sol_balance = str(position.get('total', '0'))
    except Exception:
        pass
...
def manual_buy():
    ...
    try:
        ...
        return jsonify({'status':'success','message':f'Manual buy successful: {amount} SOL for ${usdt_needed:.2f}'})
...
def manual_sell():
    ...
    try:
        ...
        data=ticker_result.get('data',{})
        current_price=float(data[0].get('last',0))
        expected_usdt=amount*current_price
        ...
        return jsonify({'status':'success','message':f'Manual sell successful: {amount} SOL for ${expected_usdt:.2f}'})
